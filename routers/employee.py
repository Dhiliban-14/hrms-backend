...
@router.get("/me/documents/{doc_type}")
def download_assigned_document(doc_type: str, employee = Depends(get_current_employee)):
    """
    Simulates downloading a generated PDF of the requested type.
    """
    buffer = io.BytesIO()
    title = doc_type.replace("-", " ").title()
    content = f"ZeAI Soft HRMS\n====================\nDocument: {title}\nEmployee: {employee['full_name']}\nID: {employee['employee_id']}\nDate: {datetime.now().date().isoformat()}\nStatus: ACCEPTED/ACTIVE\n\nThis is a mock document generated by the system."
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": f"attachment; filename={doc_type}.txt"}
    )

@router.get("/me/verification-letters/download/{request_id}")
def download_verification_letter(request_id: str, employee = Depends(get_current_employee)):
    """
    Simulates downloading a verification letter.
    """
    buffer = io.BytesIO()
    content = f"ZeAI Soft HRMS\n====================\nDocument: Employment Verification Letter\nRequest ID: {request_id}\nEmployee: {employee['full_name']}\nID: {employee['employee_id']}\nDate: {datetime.now().date().isoformat()}\nStatus: Approved & Generated\n\nThis is a mock Employment Verification Letter requested for external purposes."
    buffer.write(content.encode("utf-8"))
    buffer.seek(0)
    ...
    return StreamingResponse(
        buffer,
        media_type="text/plain",
        headers={"Content-Disposition": f"attachment; filename=Verification_Letter_{request_id}.txt"}
    )
